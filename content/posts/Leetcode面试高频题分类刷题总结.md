...
受启于: https://zhuanlan.zhihu.com/p/349940945

Sat 5 Mar 2022 00:37:56 EST

本文一共200多道题, 算上一些附加的衍生题差不多有250+, 基本上很少有easy题目, 大部分都是medium, 少部分hard, 足够应对大部分的算法面试了。

------

**排序类 (Sort) : **

- 基础知识: Quick Sort & Merge Sort 的实现, Time Complexity, In-place, Stability, Performance.
...
-   Quick Selection, Heapsort. 知道两者的取舍, 知道 C++ STL 中两个对应 method. 
    手写快速选择有时候是必要的.
    -   Leetcode 215, 347, 2099. 这三题有清晰的递进关系.
...
- 基础知识: 请熟读 *Introduction of Algorithm* 相关章节.
- Queue题目: 
...
- 基础知识: 
  ...
  常见的BFS用来解决什么问题？(1) 简单图 (有向无向皆可) 的最短路径长度, 注意是长度而不是具体的路径  (2)  遍历一个图 (或者树) 
...
以上内容皆为面试中高频的知识点. 以下知识点和题目在面试中属于中等频率.

**并查集 (Union Find) : 把两个或者多个集合合并为一个集合**

- 以近乎 $O(1)$ 的时间进行归类. UNITE, FIND, 以及两大优化; rank 的精确定义.
...
- 多数情况下可以通过用一个set来记录所有单词的prefix来替代, 时间复杂度不变, 空间复杂度提升.
- 常见题目: 
...
**单调栈 (Monotone Stack／Queue) **

- 单调的意思是保留在栈或者队列中的数字是单调递增或者单调递减的. 参见: "*Monotone Stack*".
- 常见题目: 
...
- 基础知识: 这里指的是用for循环方式的动态规划, 非Memoization Search方式。DP可以在多项式时间复杂度内解决DFS需要指数级别的问题。常见的题目包括找最大最小, 找可行性, 找总方案数等, 一般结果是一个Integer或者Boolean。动态规划有很多分支.
- 常见题目: 
